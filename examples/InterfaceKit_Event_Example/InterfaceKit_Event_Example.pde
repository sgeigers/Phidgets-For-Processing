/*
 ...
 digitalInputChange(InterfaceKit ifk)  // an event function called when a digital input is changed
*/
 ...
 *** For connecting more than one board of the same type (e.g interface kit) to a computer, see example "Connect_Multiple_InterfaceKits".
 ...
 void digitalInputChange(InterfaceKit ifk) - An event function. see full explanation bellow
 
 ifk.input - position of digital input that caused the event (works only inside the "digitalInputChange" function)

 ifk.state - new state of digital input that caused the event (works only inside the "digitalInputChange" function)
